...
class general(commands.Cog, name="general"):
    ...
    @commands.has_permissions(administrator=True)
    async def add_role(self, context, member: discord.Member, role):
        try:
    ...
    # payment methods
    @commands.command(name="payments", description="Get the payment methods")
    @commands.has_permissions(administrator=True)
    async def payments(self, context):
        embed = discord.Embed(title="Payments",
                              description=":4_paypal: - Paypal - `melchels@yahoo.com` (Friends & Family)\n"
                                          ":3_venmo: - Venmo - `@mrbyl`\n"
                                          ":2_bitcoin: - BitCoin - contact @Vert\n"
                                          ":1_cashapp: - CashApp - `$bouncedog`\n",
                              color=0xe1ff00
                              )
        try:
            await context.send(embed=embed)
